...
predictivity_getter = function(var_Y, var_X, data) {
  ## this makes no difference if we're z-scoring within run
...
Generate plots
```{r}
generate_lagged_predictivity_plots = function(vars_of_interest, lags, data) {
  plot_d = data.frame()
  ...
  p = ggplot(plot_d, aes(x=lag, y=abs(predictivity))) +
    geom_line() +
    geom_point() +
...
generate_lagged_predictivity_plots(c("y","x"), 0:2, dummy_d_lagged)
```
