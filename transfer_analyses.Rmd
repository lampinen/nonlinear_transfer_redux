...
part_summarized_simil_d = simil_d %>%
  mutate(type= case_when(rep_j == rep_i + 3 ~ "between_analog", 
                         ...
                         T ~ "within_d2"),
         abs_cosine=abs(1-cosine_similarity)) %>%
...
summarized_simil_d = part_summarized_simil_d %>%
  ...
  summarize(mean_abs_cosine = mean(mean_abs_cosine),
            mean_cosine_similarity = mean(mean_cosine_similarity))
...
```{r}
ggplot(final_simil_d, aes(x=mean_cosine_similarity, color=analogous)) +
  geom_density() +
  scale_color_brewer(palette="Set1") +
  facet_grid(~type)
```
...
learned_and_features_d = learned_d %>%
  ...
  left_join(final_sv_d %>%
              filter(mode_rank==0)) %>%
  left_join(final_simil_d %>% 
              select(-mean_abs_cosine) %>%
              mutate(type=paste("cosine_similarity_", type, sep="")) %>%
              spread(type, mean_cosine_similarity)) %>%
  left_join(final_proj_d %>%
              filter(mode_rank==0))
```

```{r}
model = lmer(learned_epoch ~ analogous + S + cosine_similarity_between_analog + cosine_similarity_between_nonanalog + abs_proj_diff + (1 | run), learned_and_features_d)
